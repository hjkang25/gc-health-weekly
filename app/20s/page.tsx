...
const C = {
  bg: '#ffffff', surface: '#f8f7ff', primary: '#5b21b6',
  accent: '#7c3aed', bright: '#a78bfa', red: '#E4002B',
  green: '#059669', yellow: '#d97706', muted: '#9ca3af',
  border: '#ede9fe', text: '#1e1b4b', textSub: '#6b7280',
};

const KEYWORDS = [
  { rank: 1, kw: '피부 트러블',  trend: 'up',   related: ['피부과 추천', '트러블 원인', '피부 진정'] },
  { rank: 2, kw: '다이어트',     trend: 'up',   related: ['간헐적 단식', '저탄고지', '체중 감량'] },
  { rank: 3, kw: '불면증',       trend: 'new',  related: ['잠 못 자는 이유', '수면제', '수면 유도'] },
  { rank: 4, kw: '탈모',         trend: 'up',   related: ['탈모 원인', '탈모 샴푸', '두피 관리'] },
  { rank: 5, kw: '우울감',       trend: 'same', related: ['우울증 증상', '무기력증', '상담'] },
  { rank: 6, kw: '헬스·운동',    trend: 'up',   related: ['단백질', '홈트', '헬스장'] },
  { rank: 7, kw: '소화불량',     trend: 'down', related: ['소화제', '속쓰림', '과민성 장'] },
  { rank: 8, kw: '눈 충혈·피로', trend: 'up',   related: ['안구건조증', '눈 피로', '루테인'] },
];

const NAVER_CATEGORIES = [
  { label: '다이어트·체형관리', ratio: 38.4 },
  { label: '피부·뷰티',         ratio: 26.1 },
  { label: '정신건강·멘탈',     ratio: 17.3 },
  { label: '운동·헬스',         ratio: 12.5 },
  { label: '수면·피로',         ratio: 5.7  },
];
    ...
    <div style={{ minHeight: '100vh', background: '#ffffff', fontFamily: SANS, color: '#1e1b4b' }}>
      <header style={{ background: '#5b21b6' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 28px', borderBottom: '1px solid rgba(255,255,255,0.15)' }}>
          <span style={{ fontFamily: MONO, fontSize: 10, color: 'rgba(255,255,255,0.5)', letterSpacing: 2 }}>2026.03.03</span>
        ...
        <div style={{ textAlign: 'center', padding: '32px 28px 24px' }}>
          ...
          <div style={{ fontFamily: SANS, fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>
            데이터로 읽는 20대의 한 주 · 2026년 3월 1주차 · VOL.10
          </div>
        </div>
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '24px 24px 80px' }}>
              ...
              {NAVER_CATEGORIES.map((c, i) => (
                <div key={c.label} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ height: 5, background: '#ede9fe', borderRadius: 2 }}>
                    <div style={{ height: '100%', width: `${c.ratio}%`, background: i === 0 ? '#7c3aed' : '#a78bfa88', borderRadius: 2 }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ background: '#f8f7ff' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', borderBottom: '2px solid #5b21b6' }}>
            ...
            <div style={{ background: '#7c3aed', padding: '20px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: '#ffffff' }}>
                {KEYWORDS.slice(0, 3).map(k => k.kw).join(' · ')}
              </div>
            </div>
          </div>
          <div style={{ padding: '24px' }}>
            ...
            {hasChecked ? (
              <>
                {SUBJECTS.filter(s => checked[s.id]).map(sub => {
                  const gs = gradeStyle(sub.grade);
                  return (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16, paddingTop: 16, borderTop: '2px solid #5b21b6' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <p style={{ fontSize: 12, color: '#6b7280', margin: '4px 0 0' }}>
                      {parseFloat(myGpa ?? '0') >= 80 ? '🎉 갓생 인증! 이 루틴 유지하세요.'
                        : parseFloat(myGpa ?? '0') >= 60 ? '😐 절반의 갓생. 수면부터 챙기세요.'
                        : '🚨 갓생 위기. 딱 하나만 고쳐볼까요?'}
                    </p>
                  </div>
                </div>
        ...
        <div style={{ background: '#5b21b6', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', marginTop: 2 }}>
          <span style={{ fontFamily: MONO, fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>갓생 건강 성적표 · 2026년 3월 1주차</span>
